...
{
$sig_response = $_POST['sig_response'];
$authenticated_username = \Duo\Web::verifyResponse('%DUO_i_KEY%','%DUO_s_KEY%','%DUO_a_KEY%',$sig_response);
}



if(isset($_POST['login']) || !empty($authenticated_username))
{
	// Try to take login
	if (!empty($authenticated_username))
	{
		$res = $db->query('SELECT uid FROM users WHERE name = ?', 's', $authenticated_username);
		$h = $db->fetch($res);
		...
		logger(LOGDEBUG, "User {$_SESSION['username']} has logged in.", __FILE__, __LINE__);
		$out->redirect($url);
	}
	else
	{
		...
		{
			// duo web 2FA
			$sig_request = \Duo\Web::signRequest('%DUO_i_KEY%','%DUO_s_KEY%','%DUO_a_KEY%',$_POST['username']);
			$out->content = '<center><iframe id="duo_iframe" width="620" height="330" frameborder="0"></iframe></center>';
			$out->content .= '<form method="POST" id="duo_form"><input type="hidden" name="stay" value="'.$_POST['stay'].'" /></form>';
			$out->content .= '<script src="js/Duo-Web-v2.min.js"></script>';
			$out->content .="<script>";
			$out->content .=" Duo.init({";
			$out->content .="'host': '%DUO_API_HOST%',";
			$out->content .="'sig_request': '$sig_request',";
			$out->content .="'post_action': '%DUO_POST_ACTION_URL%'";
			$out->content .="});";
			$out->content .="</script>";
